...
1. **Create-then-extract**: `CreateVisiblePosition(pos).DeepEquivalent()` - Creates VP only to immediately extract the Position back. Replace with `CanonicalPositionOf(pos)` or just use `pos` directly.
